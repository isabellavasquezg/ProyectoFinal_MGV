...
class Equipo(models.Model):
    ...
    sede = models.ForeignKey(Sede, on_delete=models.SET_NULL, null=True, related_name='equipos_en_sede')
    servicio = models.ForeignKey(Servicio, on_delete=models.SET_NULL, null=True, related_name='equipos_en_servicio')
    ...
    serie = models.CharField(max_length=150, unique=True) # La serie debe ser única
    ...
    def __str__(self):
        return f"{self.nombre_equipo} - {self.codigo_inventario} (S/N: {self.serie})"

# =============================
# 3. MANTENIENTOS DE EQUIPOS
# =============================
class Mantenimiento(models.Model):
    # Solo necesitamos una FK a Equipo. La ubicación se obtiene de Equipo.
    ...
    estado_mantenimiento = models.CharField(max_length=150) # Realizado, Pendiente, Cancelado

    def __str__(self):
        return f"Mantenimiento {self.equipo.nombre_equipo} - {self.fecha_mantenimiento}"
...
class TrasladoEquipo(models.Model):
    ...
    # Se relaciona con el modelo Sede, ya que el traslado es entre ubicaciones principales
    ...
    def __str__(self):
        return f"Traslado {self.equipo.nombre_equipo} de {self.sede_origen.nombre} a {self.sede_destino.nombre}"

# =============================
# 5. REGISTRO HISTÓRICO DEL EQUIPO (Información de Adquisición/Proveedor)
# =============================
class RegistroHistorico(models.Model):
    # La información de ubicación ya está en Equipo. Solo se necesita el enlace al Equipo.
    equipo = models.ForeignKey(Equipo, on_delete=models.CASCADE, related_name='registros_historicos') 
...
class DocumentoEquipo(models.Model):
    # Relación con el equipo. Se obtiene la ubicación a través de esta FK.
    equipo = models.ForeignKey(Equipo, on_delete=models.CASCADE, related_name='documentos') 
...
class MetrologiaAdmin(models.Model):
    ...
    equipo = models.ForeignKey(Equipo, on_delete=models.CASCADE, related_name='metrologia_admin')
...
class MetrologiaTecnica(models.Model):
    # Relación con el equipo.
    equipo = models.ForeignKey(Equipo, on_delete=models.CASCADE, related_name='metrologia_tecnica')
...
class CondicionesFuncionamiento(models.Model):
    # Relación con el equipo.
    equipo = models.ForeignKey(Equipo, on_delete=models.CASCADE, null=True, related_name='condiciones_funcionamiento')
